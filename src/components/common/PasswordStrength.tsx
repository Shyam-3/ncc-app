...
import { validatePassword, PASSWORD_POLICY } from '@/shared/utils/passwordPolicy';
...
const PasswordStrength: React.FC<PasswordStrengthProps> = ({ password, show = true }) => {
  ...
  const requirements = [
    { label: `${PASSWORD_POLICY.minLength}+ characters`, met: checks.minLength },
  ...
  const barColor =
    strength <= 0.4 ? '#dc3545' :   // red
    strength <= 0.6 ? '#fd7e14' :    // orange
    strength <= 0.8 ? '#ffc107' :    // yellow
    '#198754';                        // green
    ...
    <div className="mt-2 mb-1">
      ...
      <div style={{ fontSize: '0.8rem', lineHeight: 1.6 }}>
        {requirements.map((req) => (
          <div key={req.label} style={{ color: req.met ? '#198754' : '#6c757d' }}>
            <i className={`bi ${req.met ? 'bi-check-circle-fill' : 'bi-circle'} me-1`} />
            {req.label}
          </div>
        ))}
      </div>
