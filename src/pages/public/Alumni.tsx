...
const Alumni: React.FC = () => {
  ...
  const academicYears = useMemo(() => {
    const years = new Set(profiles.map(p => p.academicYear).filter(Boolean) as string[]);
  ...
  const filtered = useMemo(() => {
    ...
    if (yearFilter !== 'ALL') list = list.filter(p => p.academicYear === yearFilter);
    return list;
  ...
  return (
    <Container className="py-5">
      ...
      <Row className="g-3 mb-4">
        ...
        <Col md={3}>
          <Form.Select value={yearFilter} onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setYearFilter(e.target.value)}>
      ...
      {loading ? (
        <div className="text-center py-5"><Spinner as="span" animation="border"  size="sm" /></div>
      ) : filtered.length === 0 ? (
              ...
              <Card className="shadow-sm h-100">
                <Card.Body>
                  ...
                  <div className="mb-2">
                    ...
                    {p.academicYear && <Badge bg="light" text="dark">{p.academicYear}</Badge>}
                  </div>
