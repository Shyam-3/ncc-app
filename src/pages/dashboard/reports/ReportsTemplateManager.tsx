...
const extractVariables = (content: string) => {
  ...
  const unique = Array.from(
    new Set(matches.map((item) => item.replace(/\s+/g, ""))),
  );
...
const ReportsTemplateManager: React.FC = () => {
  ...
  const updateCatcTemplate = (
    updater: (prev: CatcCampTemplateData) => CatcCampTemplateData,
  ) => {
    setEditor((prev) => {
  ...
  useEffect(() => {
    ...
    const loadTemplates = async () => {
      try {
        ...
        setTemplates(merged);
        const selected =
          merged.find((t) => t.id === ON_DUTY_TEMPLATE_DOC_ID) || merged[0];
        setEditor(selected);
        ...
        console.error(error);
        setLoadError(
          "Failed to load templates. Ensure Firestore rules allow access to reportTemplates for admin/superadmin.",
        );
  ...
  const handleInsertVariable = (variable: string) => {
    if (isCatcTemplate) {
      toast("Select a section field below and paste the variable there.", {
        icon: "ℹ️",
      });
      return;
    }
    ...
    try {
      ...
      const normalizedId = isDraftTemplate
        ? editor.id === ON_DUTY_TEMPLATE_DOC_ID ||
          ...
          editor.id === CATC_CAMP_TEMPLATE_DOC_ID
          ? editor.id
          : slugify(editor.id)
        : editingSourceId;
      ...
      const duplicate = templates.some(
        (template) =>
          template.id === normalizedId && template.id !== editingSourceId,
      );
      if (duplicate) {
          ...
          <div className="d-flex align-items-center">
            ...
            <div>
              ...
              <div className="small opacity-75">
                Create and manage custom templates. On-Duty tab uses the On-Duty
                template and injects dynamic values into placeholders.
              </div>
            </div>
          </div>
          <Button
        ...
        <Card.Body className="bg-light">
                    ...
                    {sortedTemplates.map((template) => (
                      ...
                      >
                        ...
                        <div className="d-flex align-items-center gap-2">
                          {template.id === ON_DUTY_TEMPLATE_DOC_ID && (
                            <Badge bg="secondary">ON-DUTY</Badge>
                          )}
                          {template.id === ON_DUTY_HEADER_TEMPLATE_DOC_ID && (
                            <Badge bg="dark">ON-DUTY HEADER</Badge>
                          )}
                          {template.id === CATC_CAMP_TEMPLATE_DOC_ID && (
                            <Badge bg="info">CATC CAMP</Badge>
                          )}
                    ...
                    <Form>
                      ...
                      {isCatcTemplate && catcTemplate && (
                        <>
                          <Alert variant="info" className="small py-2">
                            Edit CATC document text page by page. Use
                            placeholders like <code>{"{{name}}"}</code> for
                            auto-filled cadet/camp values (rendered bold and
                            underlined). Use <code>**text**</code> for bold-only
                            phrases.
                          </Alert>
                              ...
                              <Form.Group>
                                ...
                                <Form.Control
                                  ...
                                  onChange={(
                                    ...
                                    updateCatcTemplate((prev) => ({
                                      ...prev,
                                      countersignStation: e.target.value,
                                    }))
                                  }
                                />
                              </Form.Group>
                              ...
                              <Form.Group>
                                ...
                                <Form.Control
                                  ...
                                  onChange={(
                                    ...
                                    updateCatcTemplate((prev) => ({
                                      ...prev,
                                      defaultCampLocation: e.target.value,
                                    }))
                                  }
                                />
                              </Form.Group>
                          ...
                          <Tab.Container
                            activeKey={activeCatcPage}
                            onSelect={(key: string | null) =>
                              setActiveCatcPage((key as CatcPageKey) || "page1")
                            }
                          >
                            <Nav variant="tabs" className="mb-3">
                              {CATC_PAGE_KEYS.map((pageKey) => (
                                <Nav.Item key={pageKey}>
                                  <Nav.Link eventKey={pageKey}>
                                    {CATC_PAGE_LABELS[pageKey]}
                                  </Nav.Link>
                                </Nav.Item>
                              ))}
                            </Nav>
                      ...
                      {!isCatcTemplate && (
                        <Form.Group className="mb-2">
                          ...
                          {editor.id === ON_DUTY_HEADER_TEMPLATE_DOC_ID && (
                            <Alert variant="info" className="small py-2">
                              Edit only the top header area here (logo + college
                              lines). Do not add date or letter body in this
                              template.
                            </Alert>
                          )}
                          {editor.id === ON_DUTY_TEMPLATE_DOC_ID && (
                            <Alert variant="info" className="small py-2">
                              This template starts below the header. Keep date
                              on the right and "To" block on the left using HTML
                              styles (for example, a flex row).
                            </Alert>
                          )}
                          <Form.Control
                            as="textarea"
                        ...
                        <Button
                          ...
                          onClick={() => {
                            if (isCatcTemplate) {
                              setEditor((prev) => ({
                                ...prev,
                                content: serializeCatcCampTemplate(
                                  DEFAULT_CATC_CAMP_TEMPLATE,
                                ),
                              }));
                              return;
          ...
          >
            ...
            <Modal.Body>
              ...
              <strong>{editor.title || editor.id}</strong>? This action cannot
              be undone.
            </Modal.Body>
            <Modal.Footer>
              <Button
