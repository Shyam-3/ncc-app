...
const AlumniManagement: React.FC = () => {
  ...
  const fetchProfiles = async () => {
    try {
      const snap = await getDocs(
        query(collection(db, "alumniProfiles"), orderBy("createdAt", "desc")),
      );
      const fetchedProfiles = snap.docs.map((d) => ({
        id: d.id,
        ...(d.data() as AlumniProfile),
      }));
      console.log("DEBUG - Total profiles fetched:", fetchedProfiles.length);
      console.log(
        "DEBUG - Pending profiles:",
        fetchedProfiles.filter((p) => p.status === "pending"),
      );
  ...
  const filteredActive = useMemo(
    () => filterList(active),
    [active, searchTerm, divisionFilter, departmentFilter],
  );
  ...
  const handleApprove = async (profile: AlumniRow) => {
    try {
      await updateDoc(doc(db, "alumniProfiles", profile.id), {
        status: "active",
        visible: true,
      });
  ...
  const handleToggleVisibility = async (row: AlumniRow) => {
    try {
      await updateDoc(doc(db, "alumniProfiles", row.id), {
        visible: !row.visible,
      });
    ...
    <Container className="py-5">
      <Card className="shadow">
        <Card.Header className="bg-primary text-white d-flex flex-column flex-sm-row justify-content-between align-items-start align-items-sm-center gap-2">
          <h3 className="mb-0">
            <i className="bi bi-mortarboard me-2"></i>Alumni Management
          </h3>
                ...
                <tbody>
                  {pending.map((p) => (
                    <tr key={p.id}>
                      ...
                      <td>
                        <Badge bg="secondary">{p.source}</Badge>
                      </td>
                      <td>
                        <Button
                ...
                <tbody>
                  {paginatedActive.map((p) => (
                    <tr key={p.id}>
                      ...
                      <td>
                        <Badge bg={p.visible ? "success" : "secondary"}>
                          {p.visible ? "Yes" : "Hidden"}
                        </Badge>
                      </td>
                      <td className="d-flex gap-1 flex-wrap">
                        ...
                        <Button
                          ...
                          onClick={() =>
                            setConfirm({ action: "delete", payload: p })
                          }
                        >
                          Delete
                        </Button>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            {editingId ? "Edit Alumni Profile" : "Add Alumni Profile"}
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
            ...
            <Col md={6}>
              ...
              <Form.Select
                ...
                onChange={(e: React.ChangeEvent<HTMLSelectElement>) =>
                  setForm((f) => ({
                    ...f,
                    division: e.target.value as "SD" | "SW",
                  }))
                }
              >
              ...
              <Row className="g-2">
                <Col>
                  <Form.Select
                    value={form.acStart}
                    onChange={(e: React.ChangeEvent<HTMLSelectElement>) =>
                      setForm((f) => ({ ...f, acStart: e.target.value }))
                    }
                  >
                ...
                <Col>
                  <Form.Select
                    value={form.acEnd}
                    onChange={(e: React.ChangeEvent<HTMLSelectElement>) =>
                      setForm((f) => ({ ...f, acEnd: e.target.value }))
                    }
                  >
                    <option value="" disabled>
                      End Year
              ...
              <Row className="g-2">
                <Col>
                  <Form.Select
                    value={form.nccStart}
                    onChange={(e: React.ChangeEvent<HTMLSelectElement>) =>
                      setForm((f) => ({ ...f, nccStart: e.target.value }))
                    }
                  >
                    <option value="" disabled>
                      Start Year
                ...
                <Col>
                  <Form.Select
                    value={form.nccEnd}
                    onChange={(e: React.ChangeEvent<HTMLSelectElement>) =>
                      setForm((f) => ({ ...f, nccEnd: e.target.value }))
                    }
                  >
                    <option value="" disabled>
                      End Year
      ...
      >
            ...
            <>
              ...
              <div className="text-center mb-4">
                {viewProfile.photoURL ? (
                  <img
                    src={
                      viewProfile.photoURL?.includes("cloudinary.com") &&
                      viewProfile.photoURL.includes("/upload/")
                        ? viewProfile.photoURL.replace(
                            "/upload/",
                ...
                <div className="d-flex justify-content-center gap-2">
                  {viewProfile.division && (
                    <Badge
                      bg={viewProfile.division === "SD" ? "info" : "warning"}
                    >
                      {viewProfile.division}
                    </Badge>
                  )}
                  {viewProfile.department && (
                    <Badge bg="secondary">{viewProfile.department}</Badge>
                  )}
              ...
              <Row>
                ...
                <Col md={6}>
                  ...
                  <div className="mb-2">
                    ...
                    <br />
                    {viewProfile.rank ? (
                      NCC_RANKS.find((r) => r.code === viewProfile.rank)
                        ?.name || viewProfile.rank
                    ) : (
                      <span className="text-muted fst-italic">
                        Not provided
                      </span>
                ...
                <Col md={6}>
                  ...
                  <div className="mb-2">
                    <small className="text-muted">NCC Tenure</small>
                    <br />
                    {viewProfile.nccTenure || viewProfile.batchYears || (
                      <span className="text-muted fst-italic">
                        Not provided
                      </span>
                    )}
        ...
        <Modal.Footer>
          <Button variant="secondary" onClick={() => setViewProfile(null)}>
            Close
          </Button>
          <Button
            variant="outline-primary"
          ...
          <Button
            ...
            onClick={() => {
              setViewProfile(null);
              if (viewProfile)
                setConfirm({ action: "reject", payload: viewProfile });
            }}
          >
          ...
          <Button
            ...
            onClick={() => {
              setViewProfile(null);
              if (viewProfile)
                setConfirm({ action: "approve", payload: viewProfile });
            }}
          >
      ...
      <Modal show={!!confirm} onHide={() => setConfirm(null)} centered>
        ...
        <Modal.Body>
          {confirm?.action === "approve" && (
            <p>
              Approve alumni profile for{" "}
              <strong>{confirm?.payload?.name}</strong>? It will become visible
          ...
          {confirm?.action === "reject" && (
            <>
              <p>
                Reject alumni profile for{" "}
                <strong>{confirm?.payload?.name}</strong>?
              </p>
              <Alert variant="info" className="mb-0">
                <i className="bi bi-info-circle me-2"></i>
                This will permanently delete the profile and queue any uploaded
                photo for cleanup. No trace will remain.
              </Alert>
            </>
          )}
        ...
        <Modal.Footer>
          ...
          {confirm?.action === "delete" && (
            <Button
              variant="danger"
              onClick={() => handleDelete(confirm.payload)}
            >
              Delete
            </Button>
          )}
